...
feature_cols = pd.read_csv("data/processed/feature_cols.csv", header=None)[0].tolist()
feature_cols = [c for c in feature_cols if c in df.columns]
proteins = df["protein_id"].unique()
print(f"  Proteins: {len(proteins)}  |  Features: {len(feature_cols)}")
results = []
print("\nRunning LOO validation...")
...
for held_out in proteins:
    ...
    for i, target in enumerate(REGRESSION_TARGETS):
        xgb = XGBRegressor(n_estimators=200, max_depth=4, learning_rate=0.05,
            ...
            tree_method="hist", monotone_constraints=build_constraint_tuple(target, feature_cols))
        xgb.fit(X_tr, y_tr[:, i])
        xgb_ests.append(xgb)
    lgb_ests = []
    for i, target in enumerate(REGRESSION_TARGETS):
    ...
    disagreement = np.mean(np.stack([np.abs(y_r-y_x),np.abs(y_x-y_l),np.abs(y_r-y_l)],axis=0),axis=(0,2))
    ...
    for i, col in enumerate(REGRESSION_TARGETS):
        ...
        reg_metrics[f"rmse_{col}"] = round(np.sqrt(mean_squared_error(y_te[:,i], y_pred[:,i])), 3)
    results.append({"held_out_protein":held_out,"test_n":int(test_mask.sum()),
        "roc_auc":round(roc_auc,3),"pr_auc":round(pr_auc,3),
        "composite_r2":round(comp_r2,3),"agg_r2":round(agg_r2,3),
        "mean_uncertainty":round(mean_unc,4),"confidence":conf,**reg_metrics})
    print(f"  {held_out:<14} {roc_auc:>7.3f} {pr_auc:>7.3f} {comp_r2:>8.3f} {agg_r2:>7.3f}  {conf}")
results_df = pd.DataFrame(results)
results_df.to_csv("outputs/reports/stacked_validation.csv", index=False)
print(f"\n{'='*55}")
print("STACKED ENSEMBLE LOO SUMMARY")
print("="*55)
...
print(f"\nSaved: outputs/reports/stacked_validation.csv")
